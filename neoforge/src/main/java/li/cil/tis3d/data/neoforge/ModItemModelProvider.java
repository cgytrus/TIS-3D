...
public final class ModItemModelProvider extends ItemModelProvider {
    ...
    private <T extends Item> void simple(final RegistrySupplier<T> item) {
        singleTexture(item.getId().getPath(),
            ResourceLocation.parse(ITEM_FOLDER + "/generated"),
            "layer0", modLoc(ITEM_FOLDER + "/" + item.getId().getPath()));
    }

    private <T extends Item> void module(final RegistrySupplier<T> item, final String overlayTexture) {
        module(item, modLoc(overlayTexture));
    }

    private <T extends Item> void module(final RegistrySupplier<T> item, final ResourceLocation overlayTexture) {
